        ...
        if chat is not None:
            response = Message(
                ...
                Message.decode_content_to_json(
                    {"response": CreateChat.RESPONSE_CHAT_NAME_EXISTS.value}
                ),
            )
        else:
            if chat_type == 0:
                response = await asyncio.create_task(second_user_verification(second_user_id, session))
            if chat_type == 1 or response is True:
                chat = Chat(chat_name=chat_name, creator_id=creator_id, type=chat_type)
                session.add(chat)
                await session.flush()
                creator_chat_member = ChatMember(
                    user_id=creator_id, chat_id=chat.id, permissions=CreateChat.ADMIN.value
                )
                session.add(creator_chat_member)
                if chat_type == 0:
                    second_chat_member = ChatMember(
                        user_id=second_user_id, chat_id=chat.id, permissions=CreateChat.ADMIN.value,
                    )
...
async def second_user_verification(second_user_id: int, session: async_session):
    if second_user_id is None:
        response = Message(
            ...
            Protocol.EMPTY_TOKEN.value,
            Message.decode_content_to_json(
                {"response": CreateChat.RESPONSE_SECOND_USER_SPECIFIED.value}
            ),
        )
    else:
        ...
        if second_user is not None:
            ...
            response = Message(
                ...
                Protocol.EMPTY_TOKEN.value,
                Message.decode_content_to_json(
                    {"response": CreateChat.RESPONSE_SECOND_USER_NOT_FOUND.value}
                ),
            )
    return response
